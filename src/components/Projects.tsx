      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {projects.map((project, index) => (
            <div
              key={project.title}
              className={`relative h-[400px] ${project.image ? 'cursor-pointer' : ''}`}
              onClick={() => handleCardClick(index)}
            >
              <div 
                className={`flip-card-inner w-full h-full transition-transform duration-700 ${
                  flippedCard === index ? 'flip-card-flipped' : ''
                }`}
                style={{ transformStyle: 'preserve-3d' }}
              >
                {/* Front Side */}
                <div 
                  className="flip-card-face flip-card-front bg-slate-800/30 backdrop-blur-xl p-8 rounded-2xl border border-slate-700/50 hover:border-slate-600/70 transition-all duration-500 group hover:transform hover:scale-105 hover:shadow-2xl hover:shadow-blue-500/10"
                ...
                {project.image && (
                  <div 
                    className="flip-card-face flip-card-back bg-slate-800/30 backdrop-blur-xl rounded-2xl border border-slate-700/50 overflow-hidden"
                    style={{ 
